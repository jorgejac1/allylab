...
export function RepoSelectionStep({ repos, isLoading, onSelect }: RepoSelectionStepProps) {
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <div style={{
          ...
          {repos.map(repo => (
            <RepoItem key={repo.id} repo={repo} onSelect={onSelect} />
          ))}
        </div>
      )}
    </div>
...
function RepoItem({ repo, onSelect }: RepoItemProps) {
  return (
    <button
